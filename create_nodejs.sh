...
#
# This script uses the image 'diganmegiovanni/nodejs:v1' to create and start
# a container running Centos 7, nodejs and npm with | Yeoman, Gulp, Grunt, Bower, 
# Babel installed |.
#
# -----------------------------------------------------------------------------------
...
read -p "Local port[80]: " LOCAL_PORT
if ! [ -z "$LOCAL_PORT" ];
then
...
echo
docker run -t -i -p $LOCAL_NODEJS_PORT:$CONTAINER_NODEJS_PORT --name $CONTAINER_NAME -v $LOCAL_DIR_TOSHARE:/sources diganmegiovanni/nodejs:v1 /bin/bash
